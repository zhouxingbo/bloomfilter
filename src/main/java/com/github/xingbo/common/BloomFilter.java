...
package com.github.xingbo.common;

import com.github.xingbo.bitset.BaseBitSet;
...
public class BloomFilter<E> implements Cloneable, Serializable {
    ...
    public boolean contains(E element) {
        ...
        int[] hashes = MessageDigestUtils.createHashes(bytes, k);
        for (int hash : hashes) {
            if (!bitSet.get(element,Math.abs(hash % bitSetSize))) {
                return false;
            }
        }
        return true;
     ...
     */
    public boolean getBit(E element,int bit) {
        return bitSet.get(element,bit);
    }
